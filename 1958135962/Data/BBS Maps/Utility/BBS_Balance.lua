								...
								if (majList[i].civ == "CIVILIZATION_MALI" and majList[i].desert_start > 0) then
									...
									elseif (majList[i].plains > 5) then
										...
										if count_2 < 3 then
											...
											__Debug("Food balancing:", majList[i].leader, "Will grand a luxury (Plain Start)");
											AddLuxuryStarting(Map.GetPlot(majList[i].plotX,majList[i].plotY),"food")	
											elseif count_2 == 4 then
											__Debug("Food balancing:", majList[i].leader, "Will grand a luxury (Plain Start)");
											AddLuxuryStarting(Map.GetPlot(majList[i].plotX,majList[i].plotY),"plains")												
											else
...
function AddLuxuryStarting(plot, s_type)
	...
	-- Attempt to place a Maize 
		for i = 17, 0, -1 do
			...
			if (adjacentPlot ~= nil) then
				for j = 1, count do
					if(adjacentPlot:GetTerrainType() == 3 or adjacentPlot:GetTerrainType() == 0) and (adjacentPlot:GetResourceType() == -1) and adjacentPlot:GetFeatureType() == -1 and adjacentPlot:IsNaturalWonder() == false then
